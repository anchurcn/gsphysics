...
====================
*/
class PreStudioModelRenderer
{
public:
	// Construction/Destruction
	PreStudioModelRenderer(void);
	virtual ~PreStudioModelRenderer(void);

	// Initialization
	virtual void InitEx();
	void SetupBonesPlayer(cl_entity_t* pEnt, float clTime, float deltaTime, float outBoneTransform[MAXSTUDIOBONES][3][4]);
	void SetupBonesModel(cl_entity_t* pEnt, float clTime, float deltaTime, float outBoneTransform[MAXSTUDIOBONES][3][4]);
	// call right after setupbonesplayer
	void SetupBonesWeapon(int weaponmodel,float outBoneTransform[MAXSTUDIOBONES][3][4]) 
	{
		model_t* pweaponmodel = IEngineStudio.GetModelByIndex(weaponmodel);
	...
	// simply copy m_pbonetransform and m_pStudioHeader->bone info,
