...
====================
*/
void PreStudioModelRenderer::Init(void)
{
...
void PreStudioModelRenderer::InitEx()
{
	// Set up some variables shared with engine
	m_pCvarHiModels = IEngineStudio.GetCvar("cl_himodels");
	m_pCvarDeveloper = IEngineStudio.GetCvar("developer");
	...
	m_plighttransform = (float(*)[MAXSTUDIOBONES][3][4])new float[MAXSTUDIOBONES][3][4];
	m_paliastransform = (float(*)[3][4])new float[3][4];
	m_protationmatrix = (float(*)[3][4])new float[3][4];
}

/*
====================
~PreStudioModelRenderer

====================
*/
PreStudioModelRenderer::~PreStudioModelRenderer(void)
{
	delete[] m_protationmatrix;
	delete[] m_paliastransform;
	delete[] m_plighttransform;
}
/*
====================
PreStudioModelRenderer
...
====================
*/
void PreStudioModelRenderer::StudioCalcBoneAdj(float dadt, float* adj, const byte* pcontroller1, const byte* pcontroller2, byte mouthopen)
{
	int					i, j;
	float				value;
...
====================
*/
void PreStudioModelRenderer::StudioCalcBoneQuaterion(int frame, float s, mstudiobone_t* pbone, mstudioanim_t* panim, float* adj, float* q)
{
	int					j, k;
	vec4_t				q1, q2;
...
====================
*/
void PreStudioModelRenderer::StudioCalcBonePosition(int frame, float s, mstudiobone_t* pbone, mstudioanim_t* panim, float* adj, float* pos)
{
	int					j, k;
	mstudioanimvalue_t* panimvalue;
...
====================
*/
void PreStudioModelRenderer::StudioSlerpBones(vec4_t q1[], float pos1[][3], vec4_t q2[], float pos2[][3], float s)
{
	int			i;
	vec4_t		q3;
	float		s1;
...
====================
*/
mstudioanim_t* PreStudioModelRenderer::StudioGetAnim(model_t* m_pSubModel, mstudioseqdesc_t* pseqdesc)
{
...
====================
*/
void PreStudioModelRenderer::StudioPlayerBlend(mstudioseqdesc_t* pseqdesc, int* pBlend, float* pPitch)
{
...
====================
*/
void PreStudioModelRenderer::StudioSetUpTransform(int trivial_accept)
{
	int				i;
	vec3_t			angles;
	vec3_t			modelpos;
...
====================
*/
float PreStudioModelRenderer::StudioEstimateInterpolant(void)
{
	float dadt = 1.0;
...
====================
*/
void PreStudioModelRenderer::StudioCalcRotations(float pos[][3], vec4_t* q, mstudioseqdesc_t* pseqdesc, mstudioanim_t* panim, float f)
{
	int					i;
	int					frame;
	mstudiobone_t* pbone;
...
====================
*/
void PreStudioModelRenderer::StudioFxTransform(cl_entity_t* ent, float transform[3][4])
{
...
====================
*/
float PreStudioModelRenderer::StudioEstimateFrame(mstudioseqdesc_t* pseqdesc)
{
	double				dfdt, f;
...
====================
*/
void PreStudioModelRenderer::StudioSetupBones(void)
{
	int					i;
	double				f;
...
====================
*/
void PreStudioModelRenderer::StudioSaveBones(void)
{
	int		i;
...
void PreStudioModelRenderer::StudioSaveBones(studiohdr_t* phdr, float inBoneTransform[MAXSTUDIOBONES][3][4])
{
	m_pStudioHeader = phdr;
	m_pbonetransform = (float(*)[MAXSTUDIOBONES][3][4])inBoneTransform;
	int		i;
...
====================
*/
void PreStudioModelRenderer::StudioMergeBones(model_t* m_pSubModel)
{
	int					i, j;
	double				f;
	int					do_hunt = true;
...
m_fDoInterp // m_fDoInterp=1 at ctor
...
====================
*/
void PreStudioModelRenderer::SetupBonesModel(cl_entity_t* pEnt, float clTime, float deltaTime, float outPBoneTransform[MAXSTUDIOBONES][3][4])
{
	...
	m_clTime = clTime;
	m_clOldTime = m_clTime - deltaTime;
	m_pPlayerInfo = NULL;
	m_pbonetransform = (float(*)[MAXSTUDIOBONES][3][4])outPBoneTransform;
	StudioDrawModel(0);
}
model_t* GetPlayerModel(const char* modelName);
void PreStudioModelRenderer::SetupBonesPlayer(cl_entity_t* pEnt, float clTime, float deltaTime, float outPBoneTransform [MAXSTUDIOBONES][3][4])
{
	m_pbonetransform = (float(*)[MAXSTUDIOBONES][3][4])outPBoneTransform;
	m_pCurrentEntity = pEnt;
	m_nPlayerIndex = pEnt->index - 1;
	auto pplayer = IEngineStudio.GetPlayerState(m_nPlayerIndex);
	...
	m_pRenderModel = GetPlayerModel(IEngineStudio.PlayerInfo(m_nPlayerIndex)->model);
...
====================
*/
void PreStudioModelRenderer::StudioEstimateGait(entity_state_t* pplayer)
{
	float dt;
	vec3_t est_velocity;
...
====================
*/
void PreStudioModelRenderer::StudioProcessGait(entity_state_t* pplayer)
{
	mstudioseqdesc_t* pseqdesc;
	float dt;
	int iBlend;
...
int PreStudioModelRenderer::ReturnDiguisedClass(int iPlayerIndex)
{
...
int PreStudioModelRenderer::StudioDrawPlayer(int flags, entity_state_t* pplayer)
{
	...
	// setup player model may throw exception
	/*
	char tempname[128];
	m_pPlayerInfo = IEngineStudio.PlayerInfo(m_nPlayerIndex);
	sprintf_s(tempname, sizeof(tempname), "models/player/%s/%s.mdl", m_pPlayerInfo->model, m_pPlayerInfo->model);
	m_pRenderModel = IEngineStudio.Mod_ForName(tempname, true);
	*/
...
====================
*/
void PreStudioModelRenderer::StudioCalcAttachments(void)
{
	int i;
...
====================
*/
void PreStudioModelRenderer::StudioRenderModel(void)
{
...
====================
*/
void PreStudioModelRenderer::StudioRenderFinal_Software(void)
{
	int i;
...
====================
*/
void PreStudioModelRenderer::StudioRenderFinal_Hardware(void)
{
	int i;
	int rendermode;
...
====================
*/
void PreStudioModelRenderer::StudioRenderFinal(void)
{
	if (IEngineStudio.IsHardware())
	{
		StudioRenderFinal_Hardware();
